...
generate_prompt = ChatPromptTemplate.from_messages(
    [("system", """You are a helpful data analyst who generates SQL queries for users based on their questions. Assume you have a table named sales with columns product_id, product_name, and sales_amount. 
      Generate SQL query for the user question using this sales table"""),
    ("human", "Question: {question}")]
)

explain_prompt = ChatPromptTemplate.from_messages(
    [("system", "You are a helpful data analyst who briefly explains SQL queries to users."),
...
def generate_sql(state: State) -> State:
    ...
    chain = itemgetter("question") | generate_prompt | sql_llm
...
def explain_sql(state: State) -> State:
    from operator import itemgetter
    chain = itemgetter("sql_query") | explain_prompt | explain_llm
    result = chain.invoke({"sql_query": state["sql_query"]})
