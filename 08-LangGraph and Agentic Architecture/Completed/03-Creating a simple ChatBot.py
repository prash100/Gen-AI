...
def chat(role, text):
    ...
    with trace(project_name="Chatbot", name="Chatbot Graph",
               inputs={"role": role, "text": text}) as rt:
        result = (chatbot_app.invoke({"messages": [HumanMessage(text)]}) 
                  if role.lower() == "user"
                  else chatbot_app.invoke({"messages": [SystemMessage(text)]})
                  )
...
chat("user", question_1)

# COMMAND ----------

chat("user", question_2)
