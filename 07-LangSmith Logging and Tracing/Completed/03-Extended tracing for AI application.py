...
class RagRetriever():
    ...
    # Enable tracing of this function with the following parameters
    #   nputs -> number of docs, docs
    #   Outputs -> number of splits
    def split_docs(self, docs):
        from langchain_text_splitters import RecursiveCharacterTextSplitter
        with trace(name="split_docs", inputs={"doc_count": len(docs), "docs": docs}) as lst:
            splitter = RecursiveCharacterTextSplitter(chunk_size=300, separators=["\n\n\n", "\n\n"])
            splitted_docs = splitter.split_documents(docs)
            lst.end(outputs={"num_splits": len(splitted_docs)})
...
class RagRouter():
    ...
    # Enable tracing with default input and output values
    @traceable
