...
rewrite_prompt = ChatPromptTemplate.from_messages(
    [("system", """You are an AI assistant who reads the user question, understand the main question that user wants to ask.
      Rewrite the user question to provide a better question for retriever to find a relevent context."""),
     ("human", "Question: {question}")]
)
...
query_rewrite_chain = itemgetter("question") | rewrite_prompt | llm | StrOutputParser()
...
rewrite_question = query_rewrite_chain.invoke({"question": user_question})
...
search_result = retriever.invoke(rewrite_question)
print(search_result[0].page_content)
...
final_rag_chain = (
    {"context": query_rewrite_chain | retriever | join_docs,
     "question": itemgetter("question")
     } | prompt | llm | StrOutputParser()
)
