...
loader = WebBaseLoader(web_paths=["https://spark.apache.org/docs/latest/sql-performance-tuning.html",
...
query_chain = perspectives_prompt | llm | queryOutputParser
...
retriever_chain = query_chain | retriever.batch

# COMMAND ----------

retriever_chain.invoke(user_question)
...
def get_unique_list(docs):
    contents = [doc[0].page_content.strip() for doc in docs]
...
final_rag_chain = (
    {"context": itemgetter("question") | retriever_chain | get_unique_list | join_contents,
...
result = final_rag_chain.invoke({"question": user_question})
print(result)
